...
#if DRAMAP
static T reduction_dramap(T* A, unsigned int nr_elements) {
    T count = 0;
    T* vec;
    dram_ap_valloc(&vec, nr_elements);
    dram_ap_vld(A, vec, nr_elements);
    dram_ap_vredsum(&count, vec, nr_elements);
...
#if DRAMAP
    T count_dramap = reduction_dramap(A, input_size);
    printf("DRAMAP result: %lu, host result: %lu \n", count_dramap, count_host);
#endif
