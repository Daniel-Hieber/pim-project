// File: Logic Unit Class - NAND-only
// Author: Deyuan Guo
// Date: June 2023
...
class lu_nand : public lu_base
{
public:
    lu_nand() : _sa(0), _t1(0) {}
    virtual ~lu_nand() {}
    virtual std::string getName() { return "nand"; }
...
protected:
    ...
    virtual B32 int_sub(B32 a, B32 b) {
        ...
        cr.set(0, Set(1));
        for (int i = 0; i < 32; i++) {
            res.set(i, FullAdd(a[i], Not(b[i]), cr));
        }
        return res;
    }
...
private:
    ...
    bool Xor(bool rs1, bool rs2) {
        ...
        _t1 = op_move(_sa);
        _sa = op_read(rs2);
        _t1 = op_nand(_sa, _t1); // x1 = A nand B
        _sa = op_nand(_sa, _t1); // x2 = B nand x1
        bool tmp = op_write(_sa);
        _sa = op_read(rs1);
        _sa = op_nand(_sa, _t1); // x3 = A nand x1
        _t1 = op_move(_sa);
        _sa = op_read(tmp);
        _sa = op_nand(_sa, _t1); // xor = x2 nand x3
        return op_write(_sa);
    }
    ...
    bool FullAdd(bool a, bool b, B1& carry) {
        ...
        _sa = op_read(b);
        _sa = op_nand(_sa, _t1);
        bool tmp1 = op_write(_sa); // tmp1 = a nand b
        _sa = op_nand(_sa, _t1);
        bool tmp2 = op_write(_sa); // tmp2 = tmp1 nand a
        _sa = op_read(b);
        _t1 = op_move(_sa);
        _sa = op_read(tmp1);
        _t1 = op_nand(_sa, _t1); // tmp3 = tmp1 nand b
        _sa = op_read(tmp2);
        _t1 = op_nand(_sa, _t1); // tmp4 = tmp2 nand tmp3
        _sa = op_read(carry[0]);
        _sa = op_nand(_sa, _t1);
        bool tmp5 = op_write(_sa); // tmp5 = tmp4 nand cr
        _sa = op_nand(_sa, _t1);
        bool tmp6 = op_write(_sa); // tmp6 = tmp5 nand tmp4
        _sa = op_read(carry[0]);
        _t1 = op_move(_sa);
        _sa = op_read(tmp5);
        _t1 = op_nand(_sa, _t1); // tmp7 = tmp5 nand cr
        _sa = op_read(tmp6);
        _sa = op_nand(_sa, _t1); // sum = tmp6 nand tmp7
        bool sum = op_write(_sa);
        _sa = op_read(tmp1);
        _t1 = op_move(_sa);
        _sa = op_read(tmp5);
        _sa = op_nand(_sa, _t1);
    ...
    bool HalfAdd(bool a, B1& carry) {
        ...
        _sa = op_read(carry[0]);
        _sa = op_nand(_sa, _t1);
        bool tmp1 = op_write(_sa); // tmp1 = a nand cr
        _sa = op_nand(_sa, _t1);
        bool tmp2 = op_write(_sa); // tmp2 = tmp1 nand a
        _sa = op_read(tmp1);
        _t1 = op_move(tmp1);
        _sa = op_read(carry[0]);
        _t1 = op_nand(_sa, _t1); // tmp3 = tmp1 nand cr
        _sa = op_read(tmp2);
        _sa = op_nand(_sa, _t1);
        bool sum = op_write(_sa); // sum = tmp2 nand tmp3
        _sa = op_read(tmp1);
        _sa = op_nand(_sa, _sa);
        carry.set(0, _sa);
        return sum;
    ...
    virtual bool Sel(bool cond, bool rs1, bool rs2) {
        return Nand(Nand(cond, rs1), Nand(Not(cond), rs2));
    }
    ...
    virtual B64 karatsuba_sub(B64 a, B64 b, int nBit) {
        ...
        B64 res;
        B1 cr;
        res = int_sub_helper(a, b, nBit, cr);
        return res;
    }
    ...
    bool op_nand(bool val1, bool val2) { _stat["nand"]++; return !(val1 && val2); }
